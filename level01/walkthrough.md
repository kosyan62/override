...
Given 32-bit elf binary, on execution it asks for a username:
...
To gain access to the next level we can write a [simple program](Resources/main.c) that will print the level02 password's file contents to the standard output:
...
Now we need to compile this program and place it at the /tmp/a location, after that we can generate command to exploit the binary using the [pawner script](Resources/pwner.py) and execute it:
